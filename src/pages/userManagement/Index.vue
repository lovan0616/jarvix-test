...
export default {
  ...
  methods: {
          ...
          this.roleOptions = response
            .map(role => {
              return {
                value: role.id,
                key: role.name,
                name: this.getZhLocaleName(role.name)
              }
            })
        })
        .catch(() => {})
    ...
    showChangeRole (user, hasPermission) {
      ...
      const option = this.roleOptions.find(option => option.name === this.getZhLocaleName(user.role)) || user.role
